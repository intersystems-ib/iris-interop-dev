...
impl IrisTools {
    ...
    #[tool(
        description = "Run %UnitTest.Manager tests on IRIS and return structured pass/fail results. Uses pure-HTTP execution via Atelier REST — works with or without IRIS_CONTAINER. Pass ONE exact compiled class name, e.g. 'MyApp.Tests.DT.Censo2Menus', or a package PREFIX such as 'MyApp.Tests', which expands to every compiled test class under that package and runs them in one call (uses /noload automatically). Pass a directory path like 'MyApp/Tests' to load from disk. Returns suite-level summary inline plus log_id for per-test-case detail via iris_get_log. A failed run ALSO returns failed_tests: the first 10 failures as {class_name, method, failure_message, failure_location, failure_assert, failure_kind}. failure_kind is 'assert' (an assertion returned false — failure_assert names which one) or 'runtime_error' (the method ABORTED, e.g. <PROPERTY DOES NOT EXIST> — failure_assert is null and failure_message carries the ObjectScript error). It is ABSENT or null when the kind was not determined (the stdout-fallback path never sets one, and so does a red test with nothing recorded). ABSENT means UNKNOWN, NOT 'no runtime error': a missing failure_kind is not evidence the test merely failed an assert — read log_id before concluding that. Read those before calling anything else — log_id has the rest. Result fields: `completed` (the suite ran — the tool worked), `outcome` ('passed'|'failed'|'no_tests'), `tests_passed`, and `success` (==tests_passed, kept for back-compat). `outcome` NEVER reports aborts — the `errors` counter it would derive that from is always 0 on this path, so read `runtime_errors` and per-case `failure_kind` instead. `runtime_errors` counts how many of `failed` ABORTED rather than failing an assertion; 0 there means NOT COUNTED, not 'nothing trapped' (the stdout-fallback path never increments it). `failed_tests` is capped at 10 inline: `failed_tests_total` is how many failures exist and `failed_tests_truncated` is true when you were handed fewer — a short `failed_tests` with no check of those two reads as a complete list. A completed run with failed>0 is a REAL test result, NOT a tool failure — fix the test/code, don't retry the tool."
    )]
    async fn iris_test(
        &self,
...
do ##class(%UnitTest.Manager).RunTest({pattern},"{flags}","{token}")"#,
    ...
    #[tool(
        description = "Read an IRIS stream by id — size first, then up to max_chars of its content, returned EXACTLY. THE question it answers: a relay wrote an empty file, so is the body empty or was it never written? An id that does not exist reports STREAM_NOT_FOUND and says out loud that this is NOT an empty stream (the existence check is %ExistsId — %OpenId hands back a usable EMPTY object for an unknown id, which is why the two are easy to confuse); a stream that exists and holds nothing reports success with size 0 and empty: true. Read-only: nothing writes to the stream, and read position is per-instance in IRIS (measured), so inspecting does not consume what a later reader needs. `body_base64` is always the exact bytes — the read goes through %Stream.GlobalBinary, so CR, NUL and arbitrary bytes all survive, and an HL7 v2 message keeps its CR-separated segments. `body` is the same bytes as text and is present only when they are valid UTF-8; otherwise `body_not_text` says to read `body_base64`. Only max_chars are read, never the whole stream, so a multi-megabyte body is inspectable rather than failing <MAXSTRING>; `truncated` plus `size` tell you it is a prefix, and `body_incomplete` that less arrived than IRIS declared. `opened_as` is the class the read used, NOT a claim about the data: character and binary streams share storage, so no read can tell you which kind an id \"really\" holds. stream_id is the stream %Id. namespace: optional — defaults to the connection namespace."
    )]
...
{}",
    ...
    #[tool(
        description = "Inspect a SQL table: returns whether it is a class-projected table or DDL-created, the backing globals — data, index, id, and the STREAM global where a class stores its stream properties (for an HL7 message that is where the content lives, and the name is not derivable from the class: EnsLib.HL7.Message stores to ^EnsLib.H.MessageS) — and (optionally) an approximate row count. Works for both class-projected tables (with real storage globals from %Dictionary.CompiledStorage) and DDL tables (globals inferred by IRIS naming convention). Use include_row_count=true to add a COUNT(*) estimate; when the count cannot be read, `row_count` is null and `row_count_error` says why, so a null is never a claim that the table is empty. Accepts either the SQL name (Ens_Config.Item) or the CLASS name (Ens.Config.Item) — the class→table projection is resolved for you, and a miss lists the tables that do exist in that package. Call this (or docs_introspect) to discover the real schema/table/column names BEFORE iris_query, rather than guessing catalog tables."
    )]
