...
/// #294: the README Tools section must list every advertised interop tool.
///
/// Its own module at the end of the file, deliberately. Three branches inserted guards before the
/// same doc-comment anchor inside `tool_annotation_tests`, and git cannot tell additive insertions
/// apart, so merging any two of them conflicted. A guard nobody can merge is worse than no guard.
#[cfg(test)]
mod readme_tool_list_guard {
    use super::*;
