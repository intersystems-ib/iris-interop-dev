...
"
    ...
    // Interop profile (this fork's default toolset) exposes exactly the 29-tool
    // interop keep-list (INTEROP_TOOLS).
    assert_eq!(
        tool_names.len(),
        29,
        "expected the 29-tool interop profile, got {}: {:?}",
        tool_names.len(),
        tool_names
    );
    ...
    let required = [
        ...
        "iris_debug",
        "docs_introspect",
        // The count assertion above is satisfied by ANY 29 tools. This pins the one whose
