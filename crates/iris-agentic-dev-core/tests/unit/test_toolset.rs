...
/// Merged must advertise exactly 53 tools, and must stay Nostub - 8 + 4.
///
...
fn test_merged_tool_count() {
    ...
    assert_eq!(
        count, 56,
        "Merged toolset must advertise exactly 56 tools, got {}",
        count
    );
...
/// Baseline advertises 57 — the 61 the `#[tool_router]` macro registers minus the 4
/// merged-only ones.
#[test]
fn test_baseline_tool_count() {
    ...
    assert_eq!(
        n, 64,
        "Baseline must advertise exactly 64 tools (Toolset::Baseline doc comment says 64), got {}",
        n
    );
}
...
fn test_nostub_tool_count_absolute() {
    let n = IrisTools::new_with_toolset(None, Toolset::Nostub)
        ...
        .registered_tool_names()
        .len();
    assert_eq!(n, 60, "Nostub must advertise exactly 60 tools, got {}", n);
}
