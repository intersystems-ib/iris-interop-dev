...
| `IRIS_SOFT_READ_ONLY` | _(unset)_ | Ask for read-only: every MUTATING call is refused regardless of what the instance looks like. **Beats `IRIS_ALLOW_PROD`** — an explicit request is not a heuristic, so it cannot be overridden by one. The tools that answer by writing a temporary class still work (see `IRIS_STRICT_READ_ONLY`). Set to `1` |
| `IRIS_STRICT_READ_ONLY` | _(unset)_ | As above, and additionally refuses the tools that answer by PUT/compile/delete of a scratch class in `IrisDevTmp` — `iris_table_info`, `iris_production_diff`, `iris_message_body`, the `hl7_schema_*` pair and the rest of `GENERATOR_WRITE_TOOLS`. They are read-only in intent but the data they read has no SQL projection to reach it through, so strict is the level for an instance that must not be written at all. Those tools are also absent from the advertised list. Wins over `IRIS_SOFT_READ_ONLY` if both are set. Set to `1` |
| `IRIS_BLOCK_CODEGEN` | _(unset)_ | Refuse an `iris_doc` put whose class declares `CodeMode = objectgenerator`. Off by default: a generator grants no capability `iris_execute` does not already have on a write-allowed connection, and on a Live one `IRIS_ALLOW_PROD` already governs the write. Turn it on where a *later* compile, by *another* identity, is the thing you are protecting. The methods are named in `compile_time_methods` on every put either way |
